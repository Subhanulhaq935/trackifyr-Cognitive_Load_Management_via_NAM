...
import { fetchReportPayload } from '@/lib/fetchReportPayload'
...
export default function ReportsPage() {
  ...
  const onDownloadDaily = async () => {
    ...
    try {
      ...
      const { downloadDailyPdf } = await import('@/components/TrackingReportPdf')
  ...
  const onDownloadWeekly = async () => {
    ...
    try {
      ...
      const { downloadWeeklyPdf } = await import('@/components/TrackingReportPdf')
        ...
        <main className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div className="rounded-xl border border-gray-100 bg-white/80 p-6 shadow-md backdrop-blur-sm dark:border-slate-700 dark:bg-slate-900/85">
              <p className="mb-2 text-sm font-medium text-gray-600 dark:text-slate-400">{ACTIVITY_DAY_AVG_LABEL} (PKT)</p>
              <p className="text-3xl font-bold text-gray-900 dark:text-slate-100">{dayAvg}</p>
            </div>
            <div className="rounded-xl border border-gray-100 bg-white/80 p-6 shadow-md backdrop-blur-sm dark:border-slate-700 dark:bg-slate-900/85">
              <p className="mb-2 text-sm font-medium text-gray-600 dark:text-slate-400">Dominant cognitive load (today)</p>
              <p className="text-3xl font-bold text-gray-900 dark:text-slate-100">{dominantCognitiveLabel(dailyRows)}</p>
            </div>
            <div className="rounded-xl border border-gray-100 bg-white/80 p-6 shadow-md backdrop-blur-sm dark:border-slate-700 dark:bg-slate-900/85">
              <p className="mb-2 text-sm font-medium text-gray-600 dark:text-slate-400">5-minute windows (rolling 7 PKT days)</p>
